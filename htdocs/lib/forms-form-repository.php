...
class FormRepository extends RedBean_Driver_PDO{
	...
	// demo only
	public function getFormsBySpecialOrderedIds($ids=array())
	{	   	   
	   $formViewsList = new FormListViewModel();
	   $formViews = array();
	   $formViewTemplates = array();
	   $specialFormViews = array();
       $formSet =  $this->getRawFormSet();
	   $excludeFromMainList=false;
       foreach ($formSet as $form)
       {
		   $formView = FormViewModel::createBasicFromObject($form);
		   if (count($ids) > 0){
				if (in_array($form["ID"], $ids)){
					array_push($specialFormViews, $formView);
					$excludeFromMainList=true;
				}else{
					$excludeFromMainList=false;
				}
		   }
		   
		   if($excludeFromMainList===false)array_push($formViews, $formView);
	   ...
	   if (count($specialFormViews) > 0){
		   
		   foreach($specialFormViews as $fv){
			   array_unshift($formViews, $fv);
		   }
	   }
